...
const Filter = ({
  ...
  const onReset = () => {
    setLocations(
      locations.map((location) => ({ ...location, isSelected: false }))
    );
    setCategories(
  ...
  useEffect(() => {
    ...
    if (isOpenLocation) {
      ...
      setLocations((locationData || []).slice(0, 4));
    }
  }, [provinces, isOpenLocation]);
  ...
  }, [serverCategories, isOpenCategory]);

  if (!isOpen) return null;

  // Render item row for SectionList
